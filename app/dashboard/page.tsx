...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    try {
      const token = localStorage.getItem('dts_access')
      const [lbData, challengeRes] = await Promise.all([
        getLeaderboard({ limit: 5 }),
        fetch('/api/challenges').then(r => r.json()),
      ])
      setLb(lbData.entries ?? [])

      const activeList: Challenge[] = (challengeRes.data ?? []).filter((c: Challenge) => c.status === 'active')
      setChallenges(activeList)

      // Check join status for all active challenges
      const joinStatuses: Record<string, boolean> = {}
      await Promise.all(
        activeList.map(async (c: Challenge) => {
          try {
      ...
      <div className="flex items-start sm:items-center justify-between mb-5 gap-3">
        <div>
          ...
          <p className="text-[10px] font-mono text-muted mt-0.5">
            // {challenges.length > 0 ? `${challenges.length} ACTIVE CHALLENGE${challenges.length > 1 ? 'S' : ''}` : 'NO ACTIVE CHALLENGES'}
          </p>
        </div>
      ...
      <div className="mb-4 fade-up-2">
        <div className="flex items-center justify-between mb-2">
          ...
          <Link href="/challenges" className="flex items-center gap-1 text-[11px] font-mono text-accent hover:opacity-70 transition-opacity">
            All challenges <ArrowRight size={11} />
          </Link>
        </div>
        ...
        {pageLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[0,1].map(i => (
              <div key={i} className="bg-surface border border-white/[0.07] rounded-xl p-5 space-y-3">
                <div className="skeleton h-4 w-16" /><div className="skeleton h-5 w-48" />
                <div className="skeleton h-2 w-full" /><div className="skeleton h-2 w-32" />
              </div>
            ))}
          </div>
        ) : challenges.length > 0 ? (
          <div className={`grid gap-3 ${challenges.length === 1 ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'}`}>
            {challenges.map(c => (
              <ChallengeCard key={c.id} challenge={c} onJoin={() => handleJoin(c.id)} joined={joined[c.id]} />
            ))}
          </div>
        ) : (
          <div className="bg-surface border border-white/[0.07] rounded-xl p-6 flex flex-col items-center justify-center gap-2 min-h-[120px]">
            <p className="text-sm text-muted font-mono">No active challenges</p>
            {user?.role === 'admin' && (
              <Link href="/admin" className="text-xs font-mono text-accent hover:opacity-70 transition-opacity">Create one →</Link>
            )}
          </div>
        )}
      </div>
        ...
        {weeklyLoading ? (
          ...
          <div className="h-[100px] flex flex-col items-center justify-center gap-2">
            ...
            <button onClick={handleSync} disabled={syncing}
              ...
              {syncing && <Spinner size={10} />} Sync now to start tracking →
            </button>
          </div>
        ) : (
          <>
            ...
            <div className="flex justify-around pt-3 mt-2 border-t border-white/[0.05]">
              {[['Easy',weeklyData.reduce((a,d)=>a+d.easy,0),'text-accent2'],
                ...
                ['Hard',weeklyData.reduce((a,d)=>a+d.hard,0),'text-danger']].map(([l,v,c])=>(
